...
model = PPO2(CnnPolicy, env, verbose=1, tensorboard_log=f"./Tensorboard/SnapshotBreakout/",
             gamma=0.99,
             lam=.95,
             vf_coef=1,
             ent_coef=0.005,
...
while True:
    try:
        model = model.load(f"SavedModels/SnapshotBreakout", env=env, tensorboard_log=f"./Tensorboard/SnapshotBreakout/")
    except:
        pass
    model.learn(total_timesteps= 1000000)
    model.save(f"SavedModels/SnapshotBreakout")
